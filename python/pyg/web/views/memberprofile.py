...
TODO(design) : long 'location' strings get cut off
TODO(design) : blank areas (i.e. no twitch handle yet) have no indication that they are editable.
TODO(design) : avatar images scale both x + y, distorting them.  They should scale by a single value and crop instead.
TODO(design) : 'upload new avatar' is same color as bg unless hovered.
TODO(ben) : upload_avatar shows up on pages the visitor does not own
TODO(ben) : member names do not always appear correctly (esp. when programattically generated by fixtures)
